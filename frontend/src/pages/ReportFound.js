...
export default function ReportFound() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setToast({ message: 'Found item reported successfully!', type: 'success' });
      reset();
      setTimeout(() => navigate('/found'), 1800);
    } catch (err) {
  ...
  return (
    <div className="page-wrap">
      ...
      <div className="report-hero">
        <h1><CheckCircle size={32} style={{ verticalAlign: 'middle', marginRight: '10px' }} /> Report <span style={{ color: 'var(--green-dark)' }}>Found</span> Item</h1>
        <p>Help someone recover their belongings by reporting what you've found.</p>
      </div>
        ...
        <div className="form-card">
          <div className="form-row">
            <div className="form-group">
              ...
              <input name="title" className="form-input" placeholder="e.g. Blue Adidas Backpack" value={form.title} onChange={handle} />
            </div>
            <div className="form-group">
          ...
          <div className="form-group full-width">
            ...
            <textarea name="description" className="form-input form-textarea" rows={5}
              placeholder="Describe the item and where exactly you found it…"
          ...
          <div className="form-group full-width">
            ...
            >
              {preview
                ...
                : <><Upload className="upload-icon" size={32} /><span className="upload-text">Drag & drop or Click to upload a photo</span></>}
              <input type="file" id="photo-found" accept="image/*" hidden onChange={handlePhoto} />
            </label>
          </div>
          <div className="form-actions">
            <button className="btn btn-outline" onClick={reset}>Reset</button>
            <button className="btn btn-green" onClick={submit} disabled={loading}>
              {loading ? 'Submitting…' : <><Send size={18} style={{ marginRight: '8px' }} /> Submit Found Report</>}
            </button>
          </div>
        </div>
      </div>
